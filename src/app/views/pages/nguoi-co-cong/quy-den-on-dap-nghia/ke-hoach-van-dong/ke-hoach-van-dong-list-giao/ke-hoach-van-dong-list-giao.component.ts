...
@Component({
	selector: 'kt-ke-hoach-van-dong-list-giao',
	templateUrl: './ke-hoach-van-dong-list-giao.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class KeHoachVanDongListGiaoComponent implements OnInit {
	// Table fields
	...
	constructor(
		...
		private translate: TranslateService) {
			this._name = 'Kế hoạch vận động được giao';
	}
	...
	ngOnInit() {
		...
		**/
		...
		this.activateRoute.queryParams.subscribe(params => {
			...
			// First load
			this.dataSource.loadListGiao(queryParams);
		});
	...
	Create(_item: KeHoachVanDongModel) {

		this.objectService.CreateData(_item).subscribe(res => {
			this.changeDetectorRefs.detectChanges();
			if (res && res.status === 1) {
				this.layoutUtilsService.showInfo("Lưu kế hoạch thành công");
			} else {
				this.layoutUtilsService.showError(res.error.message);
			}
			this.loadDataList();
