...
export class DiagramCanVas {
	...
	constructor(el: any, nodes: any[], arrows: any[], tooltip: any, options: any) {
		this.arrNode = nodes;
	...
	// }
	DrawRect(x: number, y: number, w: number, h: number, c: any, b: any) {
	...
	wrapText(context: { fillStyle: string; measureText: (arg0: string) => any; fillText: (arg0: string, arg1: number, arg2: number) => void; }, text: string, x: number, y: number, maxWidth: number) {
	...
	setPositionTooltip(el: HTMLElement, text: string, x: number, y: number) {
		el.innerText = text;
	...
	getStartEndIndexRelativeNode(Id: number, idxRelate: number, level: number, type: number) {
		if (type == 1) {//end{
			let idx = idxRelate;
			if (level == 1 || level == -1) {
				idx = (idxRelate == 3) ? 1 : 3;
	...
	getIndexRelativeNode(level: number) {
		let pos = -1;
		if (level > 1) {
			pos = 0;
		}
	...
	setDataLine(el: {
		End: number; Level: number; Idx_Start: number; Idx_End: number; }, item_node: { Relative: number[]; Location: { x: number; y: number; }; }) {
		let line = {};
		let startPos = {};
		let arr_lines = [];
		let sign = 1;
	...
	UpdateTitleNode(selectedNode: any) {
		this.arrNode.forEach(element => {
	...
	UpdateArrow(item: any, EndNode: any, IsComeBack: boolean = false) {
		try {
	...
	AddArrow(selectedNode: any, EndNode: any, IsComeBack: boolean = false) {
		try {
	...
	drawLevelNode() {
		...
		let __calDeltaY = function (idx: any, total: any, arrow_h: any) {
			let _d = (idx / total) * arrow_h / 2;
			return _d;
		};
		let __calDeltaX = function (idx: any, total: any, widthRect: any) {
	...
	addArrowToNode(level: number, idx: number) {
	...
	checkNextNode(startNode: any, endNode: any, type: number) {
	...
	setLocationRectInLevel(level: number, idx: number) {
		let point_node;
	...
	OutputStringFakeArray(array: any[]) {
		let str = '\r\n';
