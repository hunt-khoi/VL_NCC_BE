...
export class CanvasDemoComponent implements OnInit, OnChanges {
	...
	@ViewChild("DemoCanvas", { static: true }) demoCanvas: ElementRef | undefined;
	@ViewChild("tooltip", { static: true }) tooltip: ElementRef | undefined;
	//const
	LineWidth = 1;
	GridSize = 20;
	widthheadarrow = 5;
	...
	canvas: any;
	data: any;
	formNode: FormGroup | undefined;
	disableTitle = false;
	disableArrow = false;
	selectedNode: any = null;
	diagram_canvas: DiagramCanVas | undefined;
	rR: any = {};
		...
		if (this.API) {
			...
			this.canvasDemoService.getData(this.API).subscribe(data => {
				if (data && data.status == 1) {
					this.data = data.data;
					this.canvas = this.demoCanvas ? this.demoCanvas.nativeElement : {};
					this.diagram_canvas = new DiagramCanVas(this.canvas, this.data.arrNode, this.data.arrArrow, 
	...
	ngOnInit() {
		...
		// Ép kiểu sang any để TypeScript ngừng soi xét
		const storage: any = this.TokenStorage; 
		storage.getUserRolesObject().subscribe((t: any) => {
			this.rR = t;
		});
	}
	...
	nodeClicked(node: any, _this: { selectedNode: any; changeDetectorRef: { detectChanges: () => void; }; }) {
	...
	StartDeleteArrow(item: { arrow: { Start: any; End: any; }; }) {
