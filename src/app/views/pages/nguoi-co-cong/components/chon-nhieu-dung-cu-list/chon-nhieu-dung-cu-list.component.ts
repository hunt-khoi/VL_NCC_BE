...
@Component({
	selector: 'm-chon-nhieu-dung-cu-list',
	templateUrl: './chon-nhieu-dung-cu-list.component.html',
})
export class ChonNhieuDungCuListComponent implements OnInit {
	item: any;
	dataSource: ChonNhieuDungCuListDataSource | undefined;
	...
	constructor(public dialogRef: MatDialogRef<ChonNhieuDungCuListComponent>,
	...
	ngOnInit() {
		...
		// Init DataSource
		this.dataSource = new ChonNhieuDungCuListDataSource(this.service, this.layoutUtilsService);
		this.LoadDataList();
	...
	goBack(id = 0) {
		if (this.selection.selected.length > 0 && id == 1)
			this.dialogRef.close({ done: true, itemSelected: this.selectedValue });
		else
			this.dialogRef.close({ done: false, itemSelected: [] });
	}
