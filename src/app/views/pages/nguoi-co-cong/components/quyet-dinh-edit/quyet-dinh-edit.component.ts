...
@Component({
	...
	templateUrl: './quyet-dinh-edit.component.html',
...
export class QuyetDinhEditComponent implements OnInit {
	...
	ngOnInit() {
		...
		if (this.item.Id > 0) {
			...
			this.data.objectService.getItem(this.item.Id).subscribe((res: any) => {
				...
				if (res && res.status === 1) {
					this.item = res.data;
					this.ngay1 = moment(this.item.NgayRaQuyetDinh);
	...
	createForm() {
		const temp: any = {
			SoQuyetDinh: [this.item.SoQuyetDinh],
			NgayRaQuyetDinh: [this.item.NgayRaQuyetDinh, Validators.required],
	...
	prepareCustomer(): any {
		...
		_item.ObjectType = this.item.ObjectType;
		_item.ObjectId = this.item.ObjectId;
