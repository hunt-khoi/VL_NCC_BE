...
@Component({
	selector: 'kt-ho-so-nha-o-huong-dan',
	templateUrl: './ho-so-nha-o-huong-dan-dialog.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class HoSoNhaOHoanThienDialogComponent implements OnInit {
	item: any;
	itemForm: FormGroup;
	hasFormErrors = false;
	viewLoading = false;
	...
	constructor(
		public dialogRef: MatDialogRef<HoSoNhaOHoanThienDialogComponent>,
	...
	ngOnInit() {
		...
		if (this.item.id_quytrinh_lichsu > 0) {
			this.CommonService.getDetailHuongDan(this.item.id_quytrinh_lichsu).subscribe(res => {
				if (res && res.status == 1) {
					this.item = res.data;
					this.createForm();
				} else
	...
	/** UI */
	getTitle(): string {
		let result = 'Hướng dẫn hồ sơ nhà ở';
		if (this.item.id_quytrinh_lichsu > 0)
	...
	onSubmit(value: boolean) {
		...
		if (this.item.id_quytrinh_lichsu > 0) {
			this.CommonService.updateHuongDan(_item).subscribe(res => {
				if (res && res.status == 1) {
