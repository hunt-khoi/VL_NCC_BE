...
// Services
import { ThongKeHoTroService } from '../../thong-ke-ho-tro/Services/thong-ke-ho-tro.service';
...
@Component({
	selector: 'm-thong-ke-chi-tiet',
	templateUrl: './thong-ke-chi-tiet.component.html',
...
export class ThongKeChiTietComponent implements OnInit {
	...
	constructor(public tracuuHoSoService: ThongKeHoTroService,
		...
		private translate: TranslateService) {
			...
			this._name = this.translate.instant("Thống kê chi tiết hỗ trợ");
	}
	...
	loadData() {
		...
		this.display = false;
		this.tracuuHoSoService.thongKeChiTiet(this.queryParams, this.loaitk).subscribe(res => {
			this.loadingSubject.next(false);
			this.viewLoading = false;
			if (res && res.status == 1) {
				this.dataThongKe = res.data;
				this.length = this.dataThongKe.length;
	...
	xuatDanhSach() {
		...
		this.tracuuHoSoService.exportTKChiTiet(this.queryParams, this.loaitk).subscribe(res => {
			this.loadingSubject.next(false);
