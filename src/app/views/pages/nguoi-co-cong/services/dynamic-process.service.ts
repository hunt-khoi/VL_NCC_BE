...
export class DynamicProcessService {
	...
	deleteNextStep(IdStep: any, NextStep: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		const url = `${environment.ApiRoot}/step-next/xoa?IdStep=${IdStep}&NextStep=${NextStep}`;
		return this.http.get<any>(url, { headers: httpHeaders });
	}

	getStepLite(idLuong: number): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
	...
	getAction(IdTable: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
