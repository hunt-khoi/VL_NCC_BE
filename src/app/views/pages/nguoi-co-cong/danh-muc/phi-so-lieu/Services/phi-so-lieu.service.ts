...
const API_PRODUCTS_URL = environment.ApiRoot + '/phi-so-lieu';

@Injectable()
export class PhiSoLieuServices {
    lastFilter$: BehaviorSubject<QueryParamsModel> = new BehaviorSubject(new QueryParamsModel({}, 'asc', '', 0, 10));
	ReadOnlyControl: boolean = false;

	constructor(private http: HttpClient, private httpUtils: HttpUtilsService) { }
        
	getAllItems(): Observable<PhiSoLieuModel[]> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<PhiSoLieuModel[]>(API_PRODUCTS_URL + '?more=true', { headers: httpHeaders });
    }
    ...
    Create(item: PhiSoLieuModel): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.post<any>(API_PRODUCTS_URL, item, { headers: httpHeaders });
    }
    
    Update(item: PhiSoLieuModel): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
