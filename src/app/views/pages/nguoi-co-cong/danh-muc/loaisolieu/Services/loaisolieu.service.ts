...
@Injectable()
export class loaisolieuService {
	...
	// READ
	getAllItems(): Observable<loaisolieuModel[]> {

		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<loaisolieuModel[]>(API_PRODUCTS_URL + '?more=true', { headers: httpHeaders });
	}
	...
	createloaisolieu(item): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.post<any>(API_PRODUCTS_URL, item, { headers: httpHeaders });
	}

	// UPDATE => PUT: update the product on the server
	updateloaisolieu(item: loaisolieuModel): Observable<any> {
