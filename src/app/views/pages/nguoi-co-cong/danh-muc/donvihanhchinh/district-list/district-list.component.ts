...
import { TableModel } from '../../../../../../views/partials/table/table.model';
import { TableService } from '../../../../../../views/partials/table/table.service';
import { TokenStorage } from '../../../../../../core/auth/_services/token-storage.service';
import { CookieService } from 'ngx-cookie-service';

@Component({
	selector: 'm-district-list',
...
export class districtListComponent implements OnInit {
	...
	displayedColumns = ['Id_row', 'DistrictName', 'Note', 'NguoiCapNhat', 'NgayCapNhat', 'actions'];
	...
	listprovinces: any[] = [];
	// Selection
	selection = new SelectionModel<districtModel>(true, []);
	productsResult: districtModel[] = [];
	...
	list_button: boolean;
	constructor(
		public districtService1: donvihanhchinhService,
	...
	ngOnInit() {
		...
		**/
		merge(this.sort.sortChange, this.paginator.page, this.gridService.result)
			...
			.subscribe();
		// Init DataSource
		this.dataSource = new donvihanhchinhDataSource(this.districtService1);
		let queryParams = new QueryParamsModel({});

		// Read from URL itemId, for restore previous state
		this.route.queryParams.subscribe(params => {
			queryParams = this.districtService1.lastFilterHuyen$.getValue();
			queryParams.filter['ProvinceID'] = this.filterprovinces;
			// First load
			this.dataSource.loadListDistrict(queryParams);
		});
	...
	filterConfiguration(): any {
		const filter: any = {};
		if (this.filterprovinces) {
			filter.ProvinceID = +this.filterprovinces;
		}

		if (this.gridService.model.filterText) {
			filter.DistrictName = this.gridService.model.filterText.DistrictName;
		}
		return filter;
	}

	/** Delete */
	Delete(_item: districtModel) {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.districtService1.deleteDistrict(_item.Id_row).subscribe(res => {
				if (res && res.status === 1) {
	...
	Add() {
		const districtModels = new districtModel();
