...
@Component({
	selector: 'kt-nhap-bao-hiem-don-vi',
	templateUrl: './nhap-bao-hiem-don-vi.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})
@Injectable()
export class NhapBaoHiemDonViComponent implements OnInit {
	donvi: any;
	dataTreeDonVi: any[] = [];
	lstDotTangQua: any[] = [];
	CapCoCau: number;
	...
	constructor(
		...
		private NhapBaoHiemService1: NhapBaoHiemService,
	...
	In(mau = 1) {
		// this.NhapBaoHiemService1.previewDeXuatDot(this.dot, this.donvi.ID_Goc, mau).subscribe(res => {
		// 	if (res && res.status == 1) {
		// 		let dialogRef;
		// 		if (mau > 1)
		...
		// 			if (!res) {
		// 			} else {
		// 				this.NhapBaoHiemService1.exportDeXuatDot(this.dot, this.donvi.ID_Goc, mau, mau > 1, res.loai).subscribe(response => {
	...
	InQD() {
		// this.NhapBaoHiemService1.previewQD(this.dot, this.donvi.ID_Goc).subscribe(res => {
		// 	if (res && res.status == 1) {
		// 		let dialogRef = this.dialog.open(ReviewExportComponent, { data: res.data });
		// 		dialogRef.afterClosed().subscribe(res => {
		// 			if (!res) {
		// 			} else {
		// 				this.NhapBaoHiemService1.exportQD(this.dot, this.donvi.ID_Goc, res.loai).subscribe(response => {
		// 					const headers = response.headers;
		// 					const filename = headers.get('x-filename');
		// 					const type = headers.get('content-type');
		// 					const blob = new Blob([response.body], { type });
