...
export class TKTheoDoiTuongComponent implements OnInit {
	...
	constructor(public tracuuHoSoService: SoLuongTongHopService,
		...
		private translate: TranslateService) {
			this._name = this.translate.instant("Thống kê theo đối tượng");
	...
	loadData() {
		if (this.filtertypes == 0) {
			this.layoutUtilsService.showError("Vui lòng chọn loại đối tượng");
			return;
		}
		this.queryParams = this.prepareQuery();
		this.viewLoading = true;
		this.display = false;
		this.tracuuHoSoService.tkTheoDoiTuong(this.queryParams).subscribe(res => {
	...
	getValue(item, thang) {
		...
		if (find != null) {
			return find["SL"] == '0' ? '0' : this.commonService.f_currency_V2(find["SL"]);
		}
		return '0';
	}
	...
	sumTongHuyen(thang: number = 0, isCurrency = true) {
		...
		for (var i = 0; i < this.dataThongKe.data.length; i++) {
			var huyen = this.dataThongKe.data[i];
			sum += this.getValueHuyen(huyen, thang, false);
		}
		return isCurrency ? this.commonService.f_currency_V2(sum.toString()) : sum;
	}

	getValueTinh(thang: number = 0) {
		var sum = 0;
		for (var i = 0; i < this.dataThongKe.data.length; i++) { 
	...
	xuatDanhSach() {
		...
		this.tracuuHoSoService.exportTKTheoDoiTuong(this.queryParams).subscribe(res => {
			this.loadingSubject.next(false);
