...
export class DeXuatService {
	...
	//API cho phần duyệt
	duyet(itemId: number, value: boolean, note: string): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
	...
	downloadTemplate(idDot: number): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders()
	...
	previewDeXuat(id: number, mau: number): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get(API_PRODUCTS_URL + `/get-de-xuat?id=${id}&mau=${mau}`, {
			headers: httpHeaders
		});
	}
	exportDeXuat(id: number, mau: number, inngang: boolean = true, loai: number = 1) {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
	...
	exportDeXuatDot(id: number, dv: number, mau: number, inngang: boolean = true, loai: number = 1) {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
	...
	exportExcelDeXuat(id: number): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders()
