...
@Component({
	selector: 'kt-form-mtp',
	templateUrl: './form-mtp.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormMTPComponent extends FormBaseComponent implements OnInit {
	data: any;
	...
	loadLoaiTC() {
		this.commonService.liteConstLoaiTroCap().subscribe(res => {
			if (res && res.status == 1) {
				...
				this.listLoaiTroCap = this.AllLoaiTroCap.filter(x => (x.id == 19));
			}
			this.addTC();
		})
	}
