...
import { QuaTrinhHoatDongEditComponent, TroCapRowEditComponent, DiChuyenEditComponent, ThanNhanEditComponent, DCThanNhanRowEditComponent } from '../../../../components';
...
export class FormTongHopComponent implements OnInit {
	data: any;
	...
	prepareCustomer(): HoSoNCCModel {
		...
		_item.LyDoGTYKhoa = controls.LyDoYKhoa.value;
		_item.LyDoTangTuat = controls.LyDoTangTuat.value;
		...
		_item.TGThamGiaKC = controls.ThoiGianKC.value;
		_item.BangKhenCacCap = controls.BangKhen.value;
		_item.GiayTos = [];
		...
		for (var i = 0; i < this.GiayTos.length; i++) {
			...
			if(gt.IsRequired) { //giấy tờ là bắt buộc
				...
				if ((gt.So && !gt.NgayCap) || (!gt.So && gt.NgayCap)) { //chỉ nhập 1 trường
