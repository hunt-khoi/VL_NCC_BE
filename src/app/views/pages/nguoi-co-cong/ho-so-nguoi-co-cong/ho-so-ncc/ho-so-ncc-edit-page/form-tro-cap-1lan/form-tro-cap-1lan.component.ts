...
@Component({
	selector: 'kt-form-tro-cap-1lan',
	templateUrl: './form-tro-cap-1lan.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormTroCap1LanComponent extends FormBaseComponent implements OnInit {
	data: any;
	...
	loadLoaiTC() {
		this.commonService.liteConstLoaiTroCap().subscribe(res => {
			if (res && res.status == 1) {
				...
				this.listLoaiTroCap = this.AllLoaiTroCap.filter(x => x.id == 20);
			}
			this.addTC();
		})
	}
