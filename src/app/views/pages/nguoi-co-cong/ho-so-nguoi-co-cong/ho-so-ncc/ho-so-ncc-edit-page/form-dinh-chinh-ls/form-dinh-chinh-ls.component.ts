...
import {  DCThanNhanRowEditComponent, TroCapRowEditComponent } from '../../../../components';

@Component({
	selector: 'kt-form-dinh-chinh-ls',
	templateUrl: './form-dinh-chinh-ls.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormDinhChinhLSComponent implements OnInit {
	data: any;
	...
	prepareCustomer(): HoSoNCCModel {
		...
		_item.LyDoDinhChinhTN = controls.LyDoDinhChinhTN.value
		_item.ND_CanCu = controls.NoiDungCanCu.value
	...
	prepareDC(controls): any {
		...
		{
			_item.IsDuyet = true;
			_item.Id_Xa_Old = _item.Id_Xa;
			_item.DiaChi_Old = _item.DiaChi;
	...
	prepareDinhChinh(controls) {
		...
		if (controls.HoTen_new.value)
			_item.ListColumn.push({ ColumnName: 'hoten', GiaTriCu: controls.HoTen.value, GiaTriMoi: controls.HoTen_new.value, Type: 1, });
		if (controls.NamSinh_new.value)
			_item.ListColumn.push({ ColumnName: 'NamSinh', GiaTriCu: controls.NamSinh.value, GiaTriMoi: controls.NamSinh_new.value, Type: 1, });
		return _item;
	}
	onSubmit(withBack: boolean = false, callapi: boolean = false) {
		...
		if (EditHoSoNCC == null)
			return;
		if (this.nhapTC) { //xem kĩ gán nhapTC
		...
		for (var i = 0; i < this.lstDCThanNhan.length; i++) {
			if (this.lstDCThanNhan[i].cmpRef && !this.lstDCThanNhan[i].cmpRef.hostView.destroyed) {
				...
				if (DCThanNhan.CuTruDung)
					_item.ListColumn.push({ ColumnName: 'CuTru', GiaTriCu: DCThanNhan.CuTruSai, GiaTriMoi: DCThanNhan.CuTruDung, Type: 1, });
	...
	addDCTN($event = null) {
		...
		instance.data = {
			_item: { Id: 0, Id_DoiTuongNCC: 0, showDel: false, showGhiChu: false, showCuTru: true },
		};
