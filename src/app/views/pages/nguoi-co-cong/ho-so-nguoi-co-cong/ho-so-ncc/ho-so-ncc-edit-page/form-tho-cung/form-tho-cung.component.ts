...
@Component({
	selector: 'kt-form-tho-cung',
	templateUrl: './form-tho-cung.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormThoCungComponent extends FormBaseComponent implements OnInit {
	data: any;
	...
	createForm() {
		...
		const temp: any = {
			...
			NguyenQuan2: [],
			TruQuan2: [],
			NgaySinh2: [''], //ko để nháy đơn api ko nhận
			NamSinh2: [],
			GioiTinh2: [1],
			IsChet2: [0],
			NgayChet2: [],
			...
			NoiCap9: [],
			//#endregion
			//#region 10. giấy giám định y khoa
			GiayTo10: [],
			So10: [],
			NgayCap10: [],
			NoiCap10: [],
	...
	changeNS2(isNam = false) {
		...
		if (isNam) {
			...
			let val = this.itemForm.controls.NgaySinh2.value;
			if (val) {
				let y = moment(val).get('year');
				this.itemForm.controls.NamSinh2.setValue(y);
			}
		}
	}
