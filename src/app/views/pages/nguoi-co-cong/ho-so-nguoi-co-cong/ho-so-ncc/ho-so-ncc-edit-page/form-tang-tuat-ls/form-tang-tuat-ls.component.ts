...
@Component({
	selector: 'kt-form-tang-tuat-ls',
	templateUrl: './form-tang-tuat-ls.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormTangTuatLSComponent extends FormBaseComponent implements OnInit {
	data: any;
	...
	loadLoaiTC() {
		this.commonService.liteConstLoaiTroCap().subscribe(res => {
			if (res && res.status == 1) {
				...
				this.listLoaiTroCap = this.AllLoaiTroCap.filter(x => (x.id == 24 || x.id == 20));
			}
			this.addTC();
		})
	}
