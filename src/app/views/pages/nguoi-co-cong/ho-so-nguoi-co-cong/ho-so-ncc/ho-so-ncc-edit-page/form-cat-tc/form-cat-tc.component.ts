...
import { Component, OnInit, Inject, ChangeDetectionStrategy, HostListener, ViewChild, ElementRef, ChangeDetectorRef, Type, ComponentFactoryResolver, ViewContainerRef, Input } from '@angular/core';
...
@Component({
	selector: 'kt-form-cat-tc',
	templateUrl: './form-cat-tc.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormCatTCComponent extends FormBaseComponent implements OnInit {
	data: any;
	...
	loadLoaiTC() {
		this.commonService.liteConstLoaiTroCap().subscribe(res => {
			if (res && res.status == 1) {
				...
				this.listLoaiTroCap = this.AllLoaiTroCap.filter(x => (x.id == 23 || x.id == 26));
			}
			this.addTC();
		})
	}
		...
		for (var i=0; i<this.listLoaiTroCap.length; i++) {
			...
			if (item.id == 26) { //người phục vụ
				instance.data = {
					...
					allowEdit: true,
					showDel: true,
					showCat: true, //chỉ có tiền trợ cấp
				};
			}
