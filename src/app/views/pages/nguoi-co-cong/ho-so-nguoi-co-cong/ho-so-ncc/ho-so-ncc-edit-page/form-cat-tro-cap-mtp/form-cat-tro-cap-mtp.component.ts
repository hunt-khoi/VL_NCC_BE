...
@Component({
	selector: 'kt-form-cat-tro-cap-mtp',
	templateUrl: './form-cat-tro-cap-mtp.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormCatTC_MTPComponent extends FormBaseComponent implements OnInit {
	data: any;
	...
	loadLoaiTC() {
		this.commonService.liteConstLoaiTroCap().subscribe(res => {
			if (res && res.status == 1) {
				...
				this.listLoaiTroCap = this.AllLoaiTroCap.filter(x => (x.id == 19 || x.id == 20 || x.id == 23 || x.id == 26));
			}
			this.addTC();
		})
	}
		...
		for (var i=0; i<this.listLoaiTroCap.length; i++) {
			...
			if (item.id == 23) { //hàng tháng 
				instance.data = {
					...
					showDel: true,
					showCat: true,
					isPhuCap: this.item.Id_LoaiHoSo == 1 ? true : false, //1
					showNgayCat: true,
