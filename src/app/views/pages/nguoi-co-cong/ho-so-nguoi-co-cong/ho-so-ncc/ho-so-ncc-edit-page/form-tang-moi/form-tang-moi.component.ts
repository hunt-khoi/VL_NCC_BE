...
@Component({
	selector: 'kt-form-tang-moi',
	templateUrl: './form-tang-moi.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormTangMoiComponent extends FormBaseComponent implements OnInit {
	data: any;
	...
	addTC($event = null) {
		...
		for (var i=0; i<this.listLoaiTroCap.length; i++) {
			...
			if (item.id == 23) { //hàng tháng
				...
				if (this.item.Id_DoiTuongNCC == 17) {
					instance.data = { //17
						_item: { Id_LoaiTC: item.id, Title: item.title },
						allowEdit: true,
						showDel: true,
