...
@Component({
	selector: 'kt-form-cat-tc-mtp-tuat',
	templateUrl: './form-cat-tc-mtp-tuat.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormCatTC_MTP_TuatComponent extends FormBaseComponent implements OnInit {
	data: any;
	...
	ngOnInit() {
		...
		this.objectService.GetFieldTab(this.item.Id_DoiTuongNCC, this.item.Id_LoaiHoSo).subscribe(res => {
			if (res && res.status == 1) {
	...
	loadLoaiTC() {
		this.commonService.liteConstLoaiTroCap().subscribe(res => {
			if (res && res.status == 1) {
				...
				this.listLoaiTroCap = this.AllLoaiTroCap.filter(x => (x.id == 19 || x.id == 20 || x.id == 26 || x.id == 24 || x.id == 23));
			}
			this.addTC();
		})
	}
	...
	addTC($event = null) {
		...
		if (this.item.Id_DoiTuongNCC != 28 && this.item.Id_DoiTuongNCC != 29 && this.item.Id_DoiTuongNCC != 30) { 
			this.listLoaiTroCap = this.listLoaiTroCap.filter(x => x.id != 26);
		}
		for (var i=0; i<this.listLoaiTroCap.length; i++) {
			...
			if (item.id == 19) { //mtp 18, 4, 25, 28, 29, 26, 27, 30
				instance.data = {
			...
			if (item.id == 23) { //hàng tháng 
				if (this.item.Id_DoiTuongNCC == 18 || this.item.Id_DoiTuongNCC == 25 
					|| this.item.Id_DoiTuongNCC == 26 || this.item.Id_DoiTuongNCC == 27) {
					...
					instance.data = {
						...
						showCat: true,
						showNgayCat: true,
						showThangThuHoi: true
					};
				}
				if (this.item.Id_DoiTuongNCC == 4 || this.item.Id_DoiTuongNCC == 28 
					|| this.item.Id_DoiTuongNCC == 29 || this.item.Id_DoiTuongNCC == 30) {
			...
			if (item.id == 20) { //1 lần 18, 4, 25, 28, 26, 29, 27, 30
				instance.data = {
					_item: { Id_LoaiTC: item.id, Title: item.title },
					allowEdit: true,
					showDel: true,
			...
			if (item.id == 26) { //người phục vụ 28, 29, 30
				instance.data = {
					_item: { Id_LoaiTC: item.id, Title: item.title },
					allowEdit: true,
					showDel: true,
			...
			if (item.id == 24) { //tuất hàng tháng 18, 4, 25, 28, 26, 29, 27, 30
				instance.data = {
					_item: { Id_LoaiTC: item.id, Title: item.title },
					allowEdit: true,
					showDel: true,
