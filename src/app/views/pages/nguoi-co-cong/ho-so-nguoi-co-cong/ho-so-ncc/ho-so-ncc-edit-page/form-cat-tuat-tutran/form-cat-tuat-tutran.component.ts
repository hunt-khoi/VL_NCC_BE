...
@Component({
	selector: 'kt-form-cat-tuat-tutran',
	templateUrl: './form-cat-tuat-tutran.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class FormCatTuatTTComponent extends FormBaseComponent implements OnInit {
	data: any;
	...
	loadLoaiTC() {
		this.commonService.liteConstLoaiTroCap().subscribe(res => {
			if (res && res.status == 1) {
				...
				this.listLoaiTroCap = this.AllLoaiTroCap.filter(x => x.id == 24);
			}
			this.addTC();
		})
	}
