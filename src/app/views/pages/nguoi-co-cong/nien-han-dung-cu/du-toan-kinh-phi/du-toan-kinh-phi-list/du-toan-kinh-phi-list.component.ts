...
import { dutoankinhphiModel } from '../Model/du-toan-kinh-phi.model';
import { dutoankinhphiDataSource } from '../Model/data-sources/du-toan-kinh-phi.datasource';
...
@Component({
	selector: 'm-du-toan-kinh-phi-list',
	templateUrl: './du-toan-kinh-phi-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
})

export class dutoankinhphiListComponent implements OnInit {
	// Table fields
	dataSource: dutoankinhphiDataSource;
	...
	filterCondition = '';
	// Selection
	selection = new SelectionModel<dutoankinhphiModel>(true, []);
	productsResult: dutoankinhphiModel[] = [];
	_name = "";
	...
	constructor(public dutoankinhphiService1: dutoankinhphiService,
	...
	ngOnInit() {
		...
		if (this.dutoankinhphiService1 !== undefined) {
			this.dutoankinhphiService1.lastFilter$ = new BehaviorSubject(new QueryParamsModel({}, 'asc', 'Priority', 0, 10));
		...
		this.gridService.cookieName = 'displayedColumns_dutoankinhphi'

		this.gridService.showColumnsInTable();
		this.gridService.applySelectedColumnsV2(this.cookieService.check('displayedColumns_dutoankinhphi'));
		...
		// Init DataSource
		this.dataSource = new dutoankinhphiDataSource(this.dutoankinhphiService1);
		let queryParams = new QueryParamsModel({});

		// Read from URL itemId, for restore previous state
		this.route.queryParams.subscribe(params => {
			queryParams = this.dutoankinhphiService1.lastFilter$.getValue();
			// First load
	...
	*/
	...
	/** Delete */
	DeleteWorkplace(_item: dutoankinhphiModel) {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.dutoankinhphiService1.deleteItem(_item.Id).subscribe(res => {
				if (res && res.status === 1) {
	...
	AddWorkplace() {
		const dutoankinhphiModels = new dutoankinhphiModel();
		dutoankinhphiModels.clear(); // Set all defaults fields
		this.EditNhom(dutoankinhphiModels);
	}
	...
	EditNhom(_item: dutoankinhphiModel, allowEdit: boolean = true) {
		let saveMessageTranslateParam = _item.Id > 0 ? 'OBJECT.EDIT.UPDATE_MESSAGE' : 'OBJECT.EDIT.ADD_MESSAGE';
		const _saveMessage = this.translate.instant(saveMessageTranslateParam, { name: this._name });
		let dialogRef;
		if (_item.Id == 0)
			dialogRef = this.dialog.open(dutoankinhphinnewEditDialogComponent, { data: { _item, allowEdit }, width: '80%' });
		else
			dialogRef = this.dialog.open(dutoankinhphiEditDialogComponent, { data: { _item, allowEdit }, width: '80%' });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				this.loadDataList();
			}
	...
	Nhanban(_item: dutoankinhphiModel, allowEdit: boolean = true) {
		//câu thông báo khi thực hiện trong tác vụ
		let saveMessageTranslateParam = 'OBJECT.EDIT.DUPLICATE';
		let nhanban = true;
		const _saveMessage = this.translate.instant(saveMessageTranslateParam, { name: this._name });
		const dialogRef = this.dialog.open(dutoankinhphinnewEditDialogComponent, { data: { _item, allowEdit, nhanban }, width: '80%' });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				this.loadDataList();
			}
	...
	Import() {
		const dialogRef = this.dialog.open(DuToanKinhPhiImportComponent, { width: '80%' });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				return;
			}
