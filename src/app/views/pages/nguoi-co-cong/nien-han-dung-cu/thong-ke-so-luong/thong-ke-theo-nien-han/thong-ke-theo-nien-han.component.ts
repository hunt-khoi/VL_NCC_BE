...
@Component({
	selector: 'm-thong-ke-theo-nien-han',
	templateUrl: './thong-ke-theo-nien-han.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
})

export class ThongKeTheoNienHanComponent implements OnInit {
	...
	constructor(public tracuuHoSoService: ThongKeSoLuongService,
		...
		private translate: TranslateService) {
			...
			this._name = this.translate.instant("Thống kê số lượng đối tượng đến niên hạn");
	}
	...
	loadData() {
		if (this.Nam <= 0) {
			this.layoutUtilsService.showError("Vui lòng nhập năm niên hạn");
			return;
		}
		this.queryParams = this.prepareQuery();
		this.viewLoading = true;
		this.display = false;
		this.tracuuHoSoService.thongKeDenNienHan(this.queryParams).subscribe(res => {
	...
	xuatDanhSach() {
		...
		this.tracuuHoSoService.exportTKDenNienHan(this.queryParams).subscribe(res => {
			this.loadingSubject.next(false);
