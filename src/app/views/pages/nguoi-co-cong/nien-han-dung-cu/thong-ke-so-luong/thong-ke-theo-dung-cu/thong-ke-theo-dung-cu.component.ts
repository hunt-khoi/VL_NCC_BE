...
@Component({
	selector: 'm-thong-ke-theo-dung-cu',
	templateUrl: './thong-ke-theo-dung-cu.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
})

export class ThongKeTheoDungCuComponent implements OnInit {

	dataThongKe: any = { DungCus: [], data: [] };
	allowExport = false;
	...
	loadData() {
		...
		this.display = false;
		this.tracuuHoSoService.thongKeTheoDungCu(this.queryParams).subscribe(res => {
	...
	getValue(item, id_doituong) {
		let find = item.data.find(x => +x.Id_DungCu == +id_doituong);
		if (find != null)
	...
	xuatDanhSach() {
		...
		this.tracuuHoSoService.exportTKDungCu(this.queryParams).subscribe(res => {
			this.loadingSubject.next(false);
