...
@Component({
	selector: 'm-nien-han-xa-duyet-list',
	templateUrl: './nien-han-xa-duyet-list.component.html',
...
export class NienHanXaDuyetListComponent implements OnInit, OnChanges {
	// Table fields
	...
	ngOnInit() {
		...
		this.route.queryParams.subscribe(params => {
			...
			// First load
			this.dataSource.loadList(queryParams, false, true);
		});
	...
	ShowDialog() {
		...
		// 	this.DuyetDotNienHan(_item, false);
		// }
	}
	...
	DuyetDotNienHan(item: any, isDuyet: boolean = true) {
		let _item = Object.assign({}, item);
		let isxa = true;
		const dialogRef = this.dialog.open(NienHanDuyetDialogComponent, { data: { _item, isDuyet, isxa } });
	...
	detail(_item: any) {
		...
		const dialogRef = this.dialog.open(NienHanEditDialogComponent, { data: { _item, allowEdit: false, isXa }, disableClose: true });
		dialogRef.afterClosed().subscribe(res => {
		});
	}
	...
	duyets(duyet = true, isTongHop: boolean = false, isxa: boolean = false) {
	...
	timeline(QuaTrinhKhongCoNguoiDuyet: any) {
		...
		const dialogRef = this.dialog.open(SettingProcessComponent, { data: { data: data, Type: 6 } });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				return;
			}
	...
	TraLai(item: any) {
		let _item = Object.assign({}, item);
		let isxa = true;
		const dialogRef = this.dialog.open(NienHanDuyetDialogComponent, { data: { _item, isDuyet: true, isReturn: true, isxa } });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
			}
			else {
