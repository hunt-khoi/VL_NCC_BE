...
const API_ROOT_URL = environment.ApiRoot + '/chemail';
...
export class CauHinhEmailService {
	...
	getCauHinhEmailById(itemId: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<any>(API_ROOT_URL + `/detail?id=${itemId}`, { headers: httpHeaders });
	}
	deleteCauHinhEmail(itemId: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		const url = `${API_ROOT_URL}/delete?id=${itemId}`;
		return this.http.get<any>(url, { headers: httpHeaders });
	}
	deleteCauHinhEmails(ids: any[] = []): Observable<any> {
		const url = API_ROOT_URL + '';
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.post<QueryResultsModel>(url, ids, { headers: httpHeaders });
	}
	createCauHinhEmail(item: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.post<CauHinhEmailModel>(API_ROOT_URL + '/create', item, { headers: httpHeaders });
	}
	updateCauHinhEmail(item: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.post<CauHinhEmailModel>(API_ROOT_URL + '/update', item, { headers: httpHeaders });
	}
