...
export class PortletHeaderComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	@ViewChild('refIcon', {static: true}) refIcon: ElementRef | undefined;
	hideIcon: boolean = false;

	@ViewChild('refTools', {static: true}) refTools: ElementRef | undefined;
	...
	ngAfterViewInit(): void {
		...
		this.hideIcon = this.refIcon ? this.refIcon.nativeElement.children.length === 0 : false;
		// hide tools' parent node if no tools template is provided
		this.hideTools = this.refTools ? this.refTools.nativeElement.children.length === 0 : false;
