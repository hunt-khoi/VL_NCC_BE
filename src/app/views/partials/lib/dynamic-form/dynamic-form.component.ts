...
export class DynamicFormComponent implements OnInit, OnChanges, OnDestroy {
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (this.FormID && this.FormID > 0 && changes.Conditions && !changes.Conditions.firstChange) {
            for (var i = 0; i < this.item.controls.length; i++) {
                let ite = this.item.controls[i];
                if (ite.IsDepend || !ite.APIData || !ite.Condition) return;
                let _val = this.Conditions[ite.Condition];
                if (_val || _val >= 0) {
                    if (ite.DataInit != undefined) return;
                    this.dfService.getForeignKeyData(this.genAPI(ite.APIData)).subscribe(res => {
                        if (res && res.status == 1) {
                            if (ite.IdControl == 11) {
                                ite.DataInit = new BehaviorSubject<any>([]);
                                ite.DataInit.next(res.data);
                            }
                            else {
                                ite.DataInit = res.data;
                            }
    ...
    async ngOnInit() {
        if (this.FormID) {
            ...
            for (var i = 0; i < this.item.Buttons.length; i++) {
                ...
                if (button.Statement == 'list')  
                    this.item.Statement = 'list';
                if (button.IdButtonType == 0 && button.APIData && button.IdColumn) {
                    if (button.Statement != 'list')
                        this.dfService.getValueById(button.APIData, button.IdColumn, this.DataID).subscribe(res => {
                            if (res && res.status == 1) {
                                for (var i = 0; i < this.item.controls.length; i++) {
                                    let controls = this.item.controls[i];
                                    if (controls.BindData && controls.IdControl >= 0) {
                                        let prop = controls.ColumnName;
                                        let propAs = controls.ColumnNameAs;
                                        if (propAs && res.data[propAs])
                                            controls.value = res.data[propAs];
                                        else if (res.data[prop])
                                            controls.value = res.data[prop];
                                    }
                                }
                        ...
                        this.dfService.getForeignKeyData(button.APIData).subscribe(res => {
                            if (res && res.status == 1)
            ...
            for (var i = 0; i < this.item.controls.length; i++) {
                ...
                if (ite.IdControl != -2) {
                    ...
                    if (ite.IdControl == 10 || ite.IdControl == 13 || ite.IdControl == 12 || ite.IdControl == 14)
                        ite.DataInit = { Files: [], Title: ite.TenHienThi };
                    if (ite.Width > 0) {
    ...
    onSumbit(btn: any, withBack = false, reset = false) {
        ...
        const action$ = this.item.IsStatic ? this.dfService.excute(_item) : this.dfService.excuteAction(_item);
    ...
    onSumbitGroup() {
        this.dfComponents.forEach((child: any) => {
            ...
            let withBack = btn.IdButtonType == 4;
    ...
    prepareItem() {
        ...
        for (var i = 0; i < _item.controls.length; i++) {
            let controls = _item.controls[i];
            if (controls.IdControl == 0)
                controls.value = this.DataID;
            if (controls.Width > 0 && controls.NewData && controls.IdControl >= 1) {
                if (controls.IdControl == 10 || controls.IdControl == 13) {
                    if (controls[controls.IdRow].value && controls[controls.IdRow].value.length > 0)
                        controls.value = controls[controls.IdRow].value[0];
                }
                else if (controls.IdControl == 3) {
                    let _date = new Date(controls[controls.IdRow].value);
                    controls.value = this.datePipe.transform(_date, 'dd-MM-yyyy');
                }
                else {
                    controls.value = controls[controls.IdRow].value;
                }
            }
            else if (controls.IdControl == -2) {
            }
        }
        return _item;
    }
